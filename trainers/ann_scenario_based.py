...
class AnnTrainer:
    """
    Yapay sinir ağı (ANN) modelini eğitmek ve değerlendirmek için kullanılan sınıf.
    """
    def __init__(self, file_path, channels, window_size=200, cutoff=(20, 450), sampling_rate=1000, base_dir="results", patience=20, hidden_units=(128, 64), dropout_rate=0.5, learning_rate=0.001):
        """
        ...
        Parametreler:
            ...
            hidden_units (tuple): Gizli katmanlardaki nöron sayısı (örneğin, (128, 64)).
    ...
    def prepare_ann_data(self, data):
        """
        Veriyi ANN modeline uygun hale getiren metot.

        Parametreler:
            data (pd.DataFrame): Veri seti.

        Returns:
            tuple: Eğitim, doğrulama ve test için hazırlanmış veriler ve etiketler.
        """
        features = []
        labels = []
        ...
        for gesture in tqdm(unique_classes, desc="Sınıflar İşleniyor"):
            ...
            for i in tqdm(range(0, len(subset) - self.window_size, self.window_size), desc=f"Sınıf {gesture} İşleniyor", leave=False):
                ...
                # Flatten the window data for ANN input
    ...
    def build_ann_model(self, input_shape, num_classes):
        """
        ANN modelini oluşturan metot.

        Parametreler:
            input_shape (int): Giriş verisinin şekli (özellik sayısı).
            num_classes (int): Sınıf sayısı.

        Returns:
            tf.keras.models.Sequential: Derlenmiş ANN modeli.
        """
        model = Sequential()
        model.add(Dense(units=self.hidden_units[0], activation="relu", input_shape=(input_shape,)))
        model.add(Dropout(self.dropout_rate))
        for units in self.hidden_units[1:]:
           model.add(Dense(units=units, activation="relu"))
           model.add(Dropout(self.dropout_rate))

        model.add(Dense(units=num_classes, activation="softmax"))  # Softmax for multi-class
    ...
    def train_and_evaluate_model(self, save_dir, use_filter=False, use_smote=False, model_params={}, data_cleaning=False, scenario_name=""):
        ...
        Parametreler:
            ...
            use_filter (bool): Filtreleme kullanılacak mı (True/False).
            use_smote (bool): SMOTE kullanılacak mı (True/False).
            model_params (dict): ANN modelinin parametreleri.
        ...
        X_train, X_val, X_test, y_train, y_val, y_test = self.prepare_ann_data(data)
        self.log("Eğitim, Doğrulama ve Test Setleri Ayrıldı", log_file)
        
        input_shape = X_train.shape[1]
        num_classes = len(np.unique(y_train))

        model = self.build_ann_model(input_shape, num_classes)
...
if __name__ == "__main__":
    ...
    trainer = AnnTrainer(file_path="dataset/EMG-data.csv", channels=channels, window_size=200, cutoff=(20, 450), sampling_rate=1000, patience=20, hidden_units=(128, 64), dropout_rate=0.5, learning_rate=0.001)
