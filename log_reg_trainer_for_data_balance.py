...
if __name__ == "__main__":
    ...
    original_data = pd.read_csv("./dataset/EMG-data.csv")
    ...
    trainer_original = LogisticRegressionTrainer(original_data, target_column="class")
    ...
    print("\n--- SONUÇLAR ---")
    print("Dengelenmemiş Veri Seti:")
    print(f"Validation Accuracy: {results_original['val_accuracy']:.2f}")
    print(f"Test Accuracy: {results_original['test_accuracy']:.2f}")
    print("Confusion Matrix (Dengelenmemiş):")
    trainer_original.plot_confusion_matrix(results_original['confusion_matrix'], classes=np.unique(y_test), title="Confusion Matrix (Dengelenmemiş)")
