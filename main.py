...
def run_model(model_type, model_params, X_train, y_train, X_val, y_val, X_test, y_test, output_dir, filter_params, feature_params, scaler_type, balancer_type):
...
def main(file_path, selected_models, filter_params, feature_params, scaler_type="StandardScaler", balancer_type="SMOTE"):
    ...
    # Output directory'i sadece bir kere oluştur
    ...
    for model_type, model_params in selected_models:
        run_model(model_type, model_params, X_train, y_train, X_val, y_val, X_test, y_test, output_dir=output_dir, filter_params=filter_params, feature_params=feature_params, scaler_type=scaler_type, balancer_type=balancer_type)
...
if __name__ == "__main__":
    ...
    selected_models = [
        ...
        (ModelType.DECISION_TREE, {"max_depth": [5]}),
        (ModelType.RANDOM_FOREST, { "n_estimators": [100],  "max_depth": [10], "random_state": [42]}),
         (ModelType.ANN, {"hidden_layers": [[64]], "dropout_rate": [0.2], "learning_rate": [0.001], "epochs": [10], "batch_size": [32] ,
                        "early_stopping": [True],
                        "patience": [3],
                        "learning_rate_scheduling": [True],
                        "factor": [0.1],
                        "min_lr": [1e-6]}),
        (ModelType.LSTM, { "time_steps":[8] , "lstm_units": [64], "epochs": [1], "batch_size": [64] ,
                        "learning_rate": [0.001],
                        "early_stopping": [True],
                        "patience": [3],
                        "learning_rate_scheduling": [True],
    ...
    X_train, y_train, X_val, y_val, X_test, y_test = main(dataset_path, selected_models=all_model_params, filter_params=filter_params, feature_params=feature_params)
