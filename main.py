...
def run_model(model_type, model_params, X_train, y_train, X_val, y_val, X_test, y_test, output_dir):
    if model_type == ModelType.LOGISTIC_REGRESSION:
        ...
        lstm_trainer.train(
            X_train_lstm, y_train, X_val=X_val, y_val=y_val,
            epochs=model_params.get("epochs", 10),
            batch_size=model_params.get("batch_size", 32),
            learning_rate=model_params.get("learning_rate", 0.001),
            early_stopping=model_params.get("early_stopping", False),
        ...
        rf_trainer.train(X_train, y_train, X_val=X_train, y_val=y_val)
...
def main(file_path, selected_models, filter_params):
    ...
    # Veri train/validation/test ayrımı ve ölçekleme
    print("Veri train/validation/test ayrımı yapılıyor ve ölçekleniyor...")
